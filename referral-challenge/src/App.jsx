...
function LoginPage({ lang, setLang, onSuccess, onSignup }) {
  ...
  if (showForgot) {
    return <ForgotPasswordPage lang={lang} setLang={setLang} onBack={() => setShowForgot(false)} />;
  }
    ...
    <div style={S.centerWrap}>
      <div style={S.authBox}>
        ...
        <div style={{ textAlign: "right", marginTop: -6, marginBottom: 16 }}>
          <button style={{ ...S.btnText, fontSize: 12, color: C.textSoft }} onClick={() => setShowForgot(true)}>
            {t(lang, "forgotPassword")}
          </button>
        </div>

        <button style={{ ...S.btnBlueBlock, marginTop: 2 }} onClick={submit} disabled={busy}>
...
function ForgotPasswordPage({ lang, setLang, onBack }) {
  ...
  const [busy,     setBusy]     = useState(false);
  const [msg,      flash]       = useMsg();
  const [newPw,    setNewPw]    = useState("");  // 발급된 임시 비번 표시용
  ...
  async function submit() {
    ...
    try {
      ...
      // 임시 비밀번호 생성 후 DB 업데이트
      const tempPw = generateTempPw();
      const { error: updateErr } = await supabase
        .from("members")
        .update({ password: tempPw })
        .eq("id", data.id);
      ...
      <div style={S.authBox}>
        ...
        {newPw && (
          <div style={{
            ...
            <div style={{
              fontSize: 28, fontWeight: 900, color: C.blue,
        ...
        {!newPw && (
          <>
            ...
            />
            <button style={{ ...S.btnBlueBlock, marginTop: 6 }} onClick={submit} disabled={busy}>
              {busy ? <Spinner /> : t(lang, "resetPasswordBtn")}
            </button>
          </>
        )}
