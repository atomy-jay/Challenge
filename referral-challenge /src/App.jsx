...
function SignupPage({lang,setLang,onSuccess,onBack}){
  ...
  return(<div style={S.centerWrap}><div style={S.authBox}>
    ...
    <div style={S.fieldWrap}>
      ...
      <select value={form.mastership} onChange={set("mastership")} style={{...S.input,color:form.mastership?C.text:C.textSoft,cursor:"pointer"}}>
        ...
        {[
          ...
          <div key={i} style={{background:s.color+"12",border:`1.5px solid ${s.color}40`,borderRadius:12,padding:"12px 14px"}}>
            ...
            <div style={{fontWeight:700,fontSize:14,color:C.text,display:"flex",alignItems:"center",gap:6}}>
                {s.member.mastership&&<span style={{background:s.color,color:"#fff",borderRadius:8,padding:"1px 8px",fontSize:11,fontWeight:700}}>{s.member.mastership}</span>}
                {s.member.name}
              </div>
      ...
      <div style={{padding:"24px"}}>
            ...
            :currentRanking.map((m,i)=>(
              <div key={m.id} style={{display:"flex",alignItems:"center",padding:"11px 14px",borderRadius:11,marginBottom:6,background:i===0?C.blueLight:C.white,border:`1px solid ${i===0?C.blueMid:C.border}`}}>
                ...
                <div style={{flex:1,display:"flex",alignItems:"center",gap:7}}>
                  {m.mastership&&<span style={{background:C.blue,color:C.white,borderRadius:10,padding:"2px 9px",fontSize:11,fontWeight:700,flexShrink:0}}>{m.mastership}</span>}
                  <span style={{fontWeight:600,fontSize:15,color:C.text}}>{m.name}</span>
                  <span style={{fontSize:12,color:C.textSoft,marginLeft:2}}>#{m.member_no}</span>
                </div>
          ...
          <div style={{overflowX:"auto"}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{borderBottom:`2px solid ${C.border}`}}>
                {[t(lang,"memberName"),t(lang,"memberNo"),"Mastership","D","C","S",t(lang,"referralCount"),t(lang,"registeredNos")].map((h,i)=><th key={i} style={S.th}>{h}</th>)}
              </tr></thead>
              <tbody>
                {ranking.map((m,i)=>{
                  const refs=refMap[m.id]||[];
                  return(<tr key={m.id}>
                    ...
                    <td style={S.td}>{m.mastership?<span style={{background:C.blueLight,color:C.blue,border:`1px solid ${C.blueMid}`,borderRadius:10,padding:"2px 9px",fontSize:11,fontWeight:700}}>{m.mastership}</span>:<span style={{color:C.textSoft}}>—</span>}</td>
        ...
        {activeTab==="members"&&(<>
          ...
          <div style={{overflowX:"auto"}}>
            <table style={{width:"100%",borderCollapse:"collapse"}}>
              <thead><tr style={{borderBottom:`2px solid ${C.border}`}}>
                {[t(lang,"memberName"),t(lang,"memberNo"),t(lang,"email"),"Mastership","D","C","S",t(lang,"joinDate"),""].map((h,i)=><th key={i} style={S.th}>{h}</th>)}
              </tr></thead>
              <tbody>
                {members.map(m=>{
                  const refs=refMap[m.id]||[];
                  return(<tr key={m.id}>
                    ...
                    <td style={S.td}>{m.mastership?<span style={{background:C.blueLight,color:C.blue,border:`1px solid ${C.blueMid}`,borderRadius:10,padding:"2px 9px",fontSize:12,fontWeight:700}}>{m.mastership}</span>:<span style={{color:C.textSoft}}>—</span>}</td>
